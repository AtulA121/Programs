public class LongestplondronicSubstring3 {
...
//        String s="atulutatuluta";
        System.out.println(getResult(s));
    }
    public static String getResult(String s){
        String result="";
        String sss=getSubstring(s,0,s.length()-1);
        int temp2=sss.length();
        for(int i=0;i<s.length();i++){
            int temp=s.length()-1;
            String rs=sss.substring(0,temp2);
            for (int j=0;j<rs.length();j++){
                String ss=rs.substring(j);
                if(ss.equals(s.substring(i,temp+1)) && result.length() < ss.length()){
                    result=ss;
                }
                temp--;
            }
    ...
    public static String getSubstring(String s,int i,int temp){
        String result="";
